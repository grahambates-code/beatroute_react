...
const boundsNew = [
    [7.038968342537582, 46.44328103264265],
    [7.014065973317429, 47.93696136258996],
    [8.845893538374886, 47.95086223368735],
    [8.870795907595065, 46.457578766907],
];
...
export default class extends Component {
    ...
    constructor(props) {
        ...
        this.defaultViewstate = {
            map: props.card.camera || INIT_CAMERA,
            orth: {
                orbitAxis: 'Z',
                rotationX: 0,
                rotationOrbit: 0,
                target: [0, 0, 0],
                zoom: 1.5998110489966852,
            },
        };
                    ...
                    that.search(() =>
                        props.updateCard({
                            variables: {
                                ...
                                camera: this.controllerState._viewportProps,
                            },
                        }),
                    );
                }
    ...
    saveLandscapeBound = (v) => {
        ...
        this.props.updateLandscape({
            variables: { card_id: this.props.card.id, landscapecamera: bounds },
        });
        console.log('br');
        console.log(br);
    };
    ...
    render() {
        ...
        return (
            <div>
                {this.props.admin && (
                    <Buttons
                        saveLandscapeBound={this.saveLandscapeBound}
                        viewState={this.state.viewState}
                        revertView={this.revertView}
                        addLandscapeMode={this.props.addLandscapeMode}
                        refetch={this.props.refetch}
                        drawMode={this.state.drawMode}
                        setDrawMode={(dm) => this.setState({ drawMode: dm })}
                        addInk={this.state.addInk}
                        deckActive={this.props.deckActive}
                        cesiumActive={this.props.cesiumActive}
                        revert={this.revert}
                        fit={this.fit2}
                        setDeckActive={this.props.setDeckActive}
                        setCesiumActive={this.props.setCesiumActive}
                        card={this.props.card}
                        setFirstLoad={() => this.setState({ firstLoad: true })}
                        setAddInk={() => this.setState({ addInk: !this.state.addInk })}
                        fit={this.fit2}
                        setAddLandScapeMode={this.props.setAddLandScapeMode}
                    />
                )}
                ...
                <div
                    ...
                    style={{
                        ...
                        pointerEvents: this.props.deckActive ? 'all' : 'none',
                    }}
                >
                    inner
                    {true && (
                    ...
                    {false && !this.state.firstLoad && !this.state.deckActive && (
                        <div>
                            cache
                            <img
                                onClick={() => {
