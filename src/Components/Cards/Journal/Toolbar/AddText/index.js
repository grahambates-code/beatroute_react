...
const MY_MUTATION_MUTATION = gql`
    mutation MyMutationn($slide_id: Int, $data: jsonb, $scale: numeric, $rotation: numeric, $type: String, $position: jsonb, $translation: jsonb) {
        insert_asset_one(
            object: { order: 10, position: $position, rotation: $rotation, scale: $scale, slide_id: $slide_id, translation: $translation, type: $type }
        ) {
            id
        }
    }
...
const MyMutationMutation = ({ refetch, slide, type, viewState }) => {
    ...
    return (
        <Mutation mutation={MY_MUTATION_MUTATION} onCompleted={() => refetch()} variables={{ slide_id: slide.id, ...data }}>
            {(MyMutation, { loading, error, data }) => {
                ...
                return (
                    <div>
                        <button onClick={() => MyMutation()}>Add text</button>
                    </div>
                );
            }}
        </Mutation>
