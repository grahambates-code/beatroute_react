...
export default ({ viewState, slide, refetch }) => {
    ...
    const br = viewport.unproject([viewport.width / 2, viewport.width / 2], {
        topLeft: true,
    }); // make a square
        ...
        <div>
            <Mutation
                ...
                variables={{
                    ...
                    data: {
                        ...slide.data,
                        geojson: { type: 'FeatureCollection', features: [pgn] },
                    },
                }}
                onCompleted={() => {
                    //alert(refetch);
            ...
            >
                {(add, { loading, error }) => {
                    return <button onClick={() => add()}>Add slide photo</button>;
                }}
            </Mutation>
        </div>
    );
